  ...
  if (isCloudDeployed()) {
    try {
      ...
      if (fredR.ok && finR.ok) {
        ...
        window._cloudFinMindValid = !!finJ.tokenValid;
...
// FIBONACCI LEVELS
// =====================================================
/** 費波回撤：從低點 (0%) 往上加至 high (100%) */
function calcFib(high, low) {
  const diff = high - low;
  return {
    '0.0%':   { price: low,                 label: '低點 (0%)', type: 'support' },
    '23.6%':  { price: low + diff * 0.236, label: '23.6%', type: 'support' },
    '38.2%':  { price: low + diff * 0.382, label: '38.2% ⭐', type: 'key' },
    '50.0%':  { price: low + diff * 0.500, label: '50.0%', type: 'mid' },
    '61.8%':  { price: low + diff * 0.618, label: '61.8% ⭐ 黃金', type: 'key' },
    '78.6%':  { price: low + diff * 0.786, label: '78.6%', type: 'resist' },
    '100%':   { price: high,                label: '高點 (100%)', type: 'resist' },
    '127.2%': { price: high + diff * 0.272, label: '127.2% 延伸', type: 'ext' },
    '161.8%': { price: high + diff * 0.618, label: '161.8% 延伸 ⭐', type: 'ext' },
  };
}

function fibYearStartDate(ref = new Date()) {
  return `${ref.getFullYear()}-01-01`;
}

function yahooChartQuery(symbol, startDate, endDate, interval = '1d') {
  const period1 = Math.floor(new Date(`${startDate}T00:00:00`).getTime() / 1000);
  const period2 = Math.floor(new Date(`${endDate}T23:59:59`).getTime() / 1000);
  return `symbol=${encodeURIComponent(symbol)}&interval=${interval}&period1=${period1}&period2=${period2}`;
}

function extractYahooOhlcRange(json) {
  const r = json?.chart?.result?.[0];
  if (!r) return null;
  ...
  for (let i = 0; i < (r.timestamp?.length || 0); i++) {
    const h = q.high?.[i];
    const l = q.low?.[i];
    if (h == null || l == null || !(h > 0) || !(l > 0)) continue;
    hi = Math.max(hi, h);
    lo = Math.min(lo, l);
    bars++;
  }
  if (bars < 5 || !Number.isFinite(hi) || !Number.isFinite(lo) || hi <= lo) return null;
  return { high: hi, low: lo, bars };
}

function ytdRangeFromDaily(closes, timestamps) {
  ...
  for (let i = 0; i < closes.length; i++) {
    ...
    if (d.getFullYear() !== y0) continue;
    hi = Math.max(hi, c);
    lo = Math.min(lo, c);
    bars++;
  }
  if (bars < 5 || !Number.isFinite(hi) || !Number.isFinite(lo)) return null;
  return { high: hi, low: lo, bars };
}

async function fetchYahoo60mChart(symbol, startDate, endDate) {
  const q = yahooChartQuery(symbol, startDate, endDate, '60m');
  try {
    const json = await fetchYahooChartViaServer(q);
    if (json?.chart?.result?.[0]) return json;
  } catch (_) { /* fallback */ }
  ...
  const period1 = Math.floor(new Date(`${startDate}T00:00:00`).getTime() / 1000);
  const period2 = Math.floor(new Date(`${endDate}T23:59:59`).getTime() / 1000);
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${sym}?interval=60m&period1=${period1}&period2=${period2}&includePrePost=false`;
  return fetchYahooChartJson(url);
}

/** 費波基準：今年迄今 · 60 分 K 高低（失敗則改日 K） */
async function fetchFibSwingRange(ySymbol, closes, timestamps) {
  const endDate = new Date().toISOString().slice(0, 10);
  const startDate = fibYearStartDate();
  const yearLabel = String(new Date().getFullYear());

  try {
    const json = await fetchYahoo60mChart(ySymbol, startDate, endDate);
    const range = extractYahooOhlcRange(json);
    if (range) {
      return {
        ...range,
        startDate,
        endDate,
        source: `${yearLabel} 迄今 · 60分K`,
      };
    }
  } catch (e) {
    console.warn('fib 60m yahoo', ySymbol, e.message);
  }

  const daily = ytdRangeFromDaily(closes, timestamps);
  if (daily) {
    return {
      ...daily,
      startDate,
      endDate,
      source: `${yearLabel} 迄今 · 日K`,
    };
  }
...
// RENDER FIBONACCI
// =====================================================
function renderFib(fibs, currentPrice, high, low, meta = {}) {
  state.fibLevels = fibs;
  const entries = Object.entries(fibs).reverse();
  const basis = meta.source || '52週';
  const barsNote = meta.bars ? ` · ${meta.bars} 根K` : '';
  const periodNote = meta.startDate && meta.endDate ? ` · ${meta.startDate}～${meta.endDate}` : '';
  document.getElementById('fibLevels').innerHTML = `
    <div style="font-size:10px;color:var(--muted);margin-bottom:8px;">
      基準: <span class="gold">${basis}</span>${periodNote}${barsNote}<br>
      低 ${low.toLocaleString(undefined, { maximumFractionDigits: 1 })} (0%) → 高 ${high.toLocaleString(undefined, { maximumFractionDigits: 1 })} (100%) · 區間 ${(high - low).toLocaleString(undefined, { maximumFractionDigits: 1 })} · 現價 ${currentPrice.toLocaleString()}
    </div>
...
async function loadSymbol() {
  ...
  setHtml('marginPanel', '<div class="loading"><span class="spinner"></span>載入融資餘額...</div>');
  ...
  try {
    ...
    const fibRange = await fetchFibSwingRange(ySymbol, closes, timestamps);
    const fibs = calcFib(fibRange.high, fibRange.low);
    renderFib(fibs, price, fibRange.high, fibRange.low, fibRange);
    ...
    if (typeof scheduleMarginLoad === 'function') scheduleMarginLoad(rawInput);
    const marginTitle = document.getElementById('marginPanelTitle');
    if (marginTitle) {
      const k = classifySymbol(rawInput);
      marginTitle.textContent = k.kind === 'stock' ? `${k.id} 個股` : k.kind === 'taiex' ? '台股整體' : rawInput;
    }
    if (typeof calcKelly === 'function' && document.getElementById('totalCapital')) calcKelly();
...
async function bootDashboard() {
  ...
  if (typeof scheduleOptionsOiLoad === 'function') scheduleOptionsOiLoad();
  if (typeof scheduleMarginLoad === 'function') scheduleMarginLoad();
}
...
setInterval(() => {
  if (document.visibilityState === 'visible') {
    loadSymbol();
    if (typeof loadOptionsOiPanel === 'function') loadOptionsOiPanel(true);
    if (typeof loadMarginPanel === 'function') loadMarginPanel(null, true);
  }
}, 300000);
